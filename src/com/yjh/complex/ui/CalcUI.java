...
//		operationPanel.setBackground(Color.red);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource().equals(clearBtn)) {//清除
			DataCleaner.clearData();
		}
		else if(e.getSource().equals(addBtn)) {//加法
			symbolText.setText("＋");
			if(DataChecker.checkData())
				cr = CalcFactory.getInstance().getCalcRole(CalcType.ADD_CALC);
			cr.calcComplex();//计算
		}
		else if(e.getSource().equals(subBtn)) {//减法
			symbolText.setText("－");
			if(DataChecker.checkData())
				cr = CalcFactory.getInstance().getCalcRole(CalcType.SUB_CALC);
			cr.calcComplex();//计算
		}
		else if(e.getSource().equals(multBtn)) {//乘法
			symbolText.setText("×");
			if(DataChecker.checkData())
				cr = CalcFactory.getInstance().getCalcRole(CalcType.MULT_CALC);
			cr.calcComplex();//计算
		}
		else if(e.getSource().equals(divBtn)) {//乘法
			symbolText.setText("÷");
			if(DataChecker.checkData())
				cr = CalcFactory.getInstance().getCalcRole(CalcType.DIV_CALC);
			cr.calcComplex();//计算
		}
		else if(e.getSource().equals(absBtn)) {//绝对值
			symbolText.setText("||");
			cr = CalcFactory.getInstance().getCalcRole(CalcType.ABS_CALC);
			cr.calcComplex();//计算
		}
